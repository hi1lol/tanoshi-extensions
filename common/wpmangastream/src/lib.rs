...
pub fn parse_manga_list(url: &str, source_id: i64, body: &str) -> Result<Vec<MangaInfo>> {
    ...
    let selector_name =
        Selector::parse("div.bsx > a").map_err(|e| anyhow!("failed to parse selector: {:?}", e))?;
    ...
    Ok(doc
        ...
        .filter_map(|el| {
            let Some(link) = el.select(&selector_name).next() else {
                log::warn!("Skipping malformed WP Manga Stream card from {url}: missing link");
                return None;
            };
            let Some(title) = link
                .value()
                ...
                log::warn!("Skipping malformed WP Manga Stream card from {url}: missing title");
                return None;
            };
            let Some(path) = link.value().attr("href").filter(|href| !href.is_empty()) else {
                log::warn!("Skipping malformed WP Manga Stream card from {url}: missing URL");
                return None;
            };
            let Some(cover_url) = el
                ...
                log::warn!("Skipping malformed WP Manga Stream card from {url}: missing image");
                return None;
            };
